...
def skeleton_exclude_joint_indices(motion: Motion) -> list[int]:
    """Bone indices to skip in compact stick/capsule visualization."""
...
def _serialize_body_mesh(
    ...
    *,
    ...
    if _MAX_MESH_GZIP_BYTES > 0 and len(gz) > _MAX_MESH_GZIP_BYTES:
        return {
            "available": False,
            "reason": "mesh too large — use a shorter clip or disable body mesh display",
        }
    return {
        "available": True,
        "type": "baked",
...
def _serialize_terrain(terrain) -> dict[str, Any]:
    """Triangulated terrain mesh for three.js.
...
# Canonical human topology for the scaled-effector preview.
...
def object_mesh_glb(obj, *, scale: float | None = None) -> bytes | None:
    ...
    ``obj.scale``), matching OMOMO ``positions`` semantics. Pass an explicit
    ``scale`` for robot-frame scaled props (``obj.scale * uniform_ratio``).
    """
